...
/**
 * The four character rows of the Norwegian (ISO) keyboard — everything
 * except the bottom modifier row, which differs between the PC and Mac
 * variants (see `no-mac.ts`) and so is defined separately below.
 *
 ...
 */
export const NO_CHAR_ROWS: KeyDef[][] = [
	[
...
/** The bottom modifier row for the PC variant. AltGr (id `altgr`, held by
 * the right thumb) is the key `planChar` looks up by id to produce
 * AltGr-layer characters — see `engine/plan.ts`. */
export const PC_MODIFIER_ROW: KeyDef[] = [
	{
		id: 'ctrlL',
		base: null,
		shift: null,
...
export const NO_PC_LAYOUT: KeyboardLayout = {
	id: 'no-pc',
	name: 'Norsk (PC)',
	rows: [...NO_CHAR_ROWS, PC_MODIFIER_ROW]
};
